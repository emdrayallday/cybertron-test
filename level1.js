...
export default function() {
  ...
  const exercise1 = _ => {
    ...
    return map(num => num * num * num, numbers); // return answer here
  };

  const ex2 = "use filter to only return numbers divisible by 6";
  const exercise2 = _ => {
    const numbers = [28, 42, 55, 66, 72, 84, 93];
    return filter(num => num % 6 === 0, numbers); // return answer here
  };

  const ex3 = "use reduce to sum the numbers";
  const exercise3 = _ => {
    const numbers = [10, 20, 30, 40, 50, 60];
    return reduce((acc, val) => acc + val, 0, numbers); // return answer here
  };
...
`;
  ...
  const exercise5 = _ => {
    ...
    return map(num => Math.sqrt(num), numbers); // return answer here
  };

  const ex6 = "use filter to return numbers between 10 and 20";
  const exercise6 = _ => {
    const numbers = [1, 5, 6, 3, 10, 12, 18, 21, 28, 34, 39, 45];
    return filter(num => num > 10 && num < 20, numbers); // return answer here
  };
